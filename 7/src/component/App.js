    ...
    const render = () => {
        ...
        return (
            <div>
                ...
                <Switch>
                    <Route exact path='/'
                        render={() => <Welcome playerName={props.playerName} onConnect={onConnect}
                            wsConnect={state.WSConnect} wsPlayerName={state.wsPlayerName}></Welcome>}/>

                    <Route path='/createGame'>{ !state.WSConnect ? <Redirect to="/" /> :
                        <CreateGame onNewGame={onStart} enableAlertMessage={true}></CreateGame>}
                    </Route>

                    <Route path='/players'>{ !state.WSConnect ? <Redirect to="/" /> :
                        <OnlinePlayers players={state.wsPlayerList}
                            currentPlayer={{id: state.wsPlayerId, name: state.wsPlayerName}}></OnlinePlayers>}
                    </Route>

                    <Route path='/ongoingGames'>{ !state.WSConnect ? <Redirect to="/" /> :
                        <GameList onGameClick={onGameClick} inProgressMode={true}
                            gamesDic={state.gamesDic}></GameList>}
                    </Route>

                    <Route path='/finishedGames'>{ !state.WSConnect ? <Redirect to="/" /> :
                        <GameList onGameClick={onGameClick} inProgressMode={false}
                            gamesDic={state.gamesDic}></GameList>}
                    </Route>
